...
class SearchResultsPage(BasePage): 
    ...
    def get_first_result_url(self):
        return self.driver.find_element(*self.first_result_Google).get_attribute("href");

class Xataca(BasePage): 

    boton_aceptar_cookies_xataka = (By.ID, "didomi-notice-agree-button");
    primerArticulo = (By.XPATH,"//div[@id='stories_container']/following-sibling::div[1]/descendant::a[2]")
    
    def aceptar_cokies_xataka(self):
        self.driver.find_element(*self.boton_aceptar_cookies_xataka).click();
        
    def primer_articulo(self):
        self.driver.find_element(*self.primerArticulo).click();

    def get_title_primer_articulo(self):
    #     return self.driver.title;
        return self.driver.find_element(*self.primerArticulo).get_attribute("title");
